...
Directory structure: RESULTS/Task/Big_Tournament_Sizes/Selection/Rep_i/test_mse.txt
"""
...
def compile_results(base_dir='RESULTS'):
    ...
    for task_dir in sorted(base_path.iterdir()):
        ...
        # Look for Big_Tournament_Sizes directory
        all_tourney_dir = task_dir / 'Big_Tournament_Sizes'
        if not all_tourney_dir.exists() or not all_tourney_dir.is_dir():
            print(f"  Warning: Big_Tournament_Sizes directory not found in {task}")
            continue
...
Output:
  Creates a single CSV file (Big_Tournament_Sizes.csv) containing:
  Columns: Task, Selection, Rep, performance
    ...
    parser.add_argument(
        '-o', '--output',
        type=str,
        default='big_tournament_sizes.csv',
        help='Output CSV filename (default: big_tournament_sizes.csv)'
    )
